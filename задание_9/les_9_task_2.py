...
class Haffman:
    ...
    def _make_list(self, real_str):
        ...
        counter = collections.OrderedDict(
            sorted(counter.items(), key=lambda k: k[1], reverse=True))
            ...
            if spam.value > self._data[0].value:
                ...
                for i in range(1, len(self._data)):
                    if self._data[i - 1].value >= spam.value > self._data[i].value:
                        ...
                        break
        self._data = Node(
            self._data[0].value + self._data[1].value, self._data[0], self._data[1])
...
if __name__ == '__main__':
    ...
    print("Возвращает строку из '0' и '1'")
    print(haf.get_real_string_code())
    print("Cтроки преобразованые в код по алгоритму Хаффмана")
    print(code_s)
    table = haf.get_table_code()
    print("Таблица кодирования в виде словаря {буква: код} ")
    print(table)
    print("Декодирование строки из '0' и '1' на основе таблицы кодирования code_table")
    real = haf.decode(code_s, table)
    print(real)